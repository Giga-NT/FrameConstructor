...
export const useFrameModel = (
    ...
    // Константа для подъема ригелей
    const BEAM_LIFT = 0.001; // 1mm lift
    ...
    // Функция создания квадратной трубы
const createSquareBeam = (
  ...
  // Основной поворот (ось Z -> направление балки)
  mesh.quaternion.setFromUnitVectors(new THREE.Vector3(0, 0, 1), direction);

  // Дополнительные микро-повороты для верхнего пояса
  if (!alignToGround) {
    const euler = new THREE.Euler(
      0.001,  // X (если нужно наклонить вперед/назад)
      0.002,  // Y (если нужно "развернуть" грань)
      0.005,  // Z (если нужно скорректировать угол)
      'XYZ'   // Порядок осей
    );
  ...
  return mesh;
    ...
    // Функция создания плоского профиля
    ...
    const createInclinedTruss = (
        ...
        // Определяем, является ли это крайней левой или правой фермой
        ...
        // Рассчитываем количество пролетов (минимум 2)
        ...
        // Строим нижний пояс (только основную часть без свесов)
        let currentX = 0;
        ...
        // Задний свес (добавляем только к верхнему поясу)
        if (rearOverhang > 0) {
        ...
        // Левый боковой свес (только для крайней левой фермы)
        ...
        // Правый боковой свес (только для крайней правой фермы)
        ...
        // Нижний пояс (только основная часть)
        ...
        // Вертикальные стойки (соединяем с верхним поясом с учетом свесов)
    ...
    const createRoofPurlins = (
        ...
        for (let x = startX; x <= endX; x += purlinSpacing) {
            ...
            // Основной прогон между крайними фермами
    ...
    // Основная функция создания модели
    const model = useMemo(() => {
        ...
        // Горизонтальные балки между колоннами (с подъемом)
