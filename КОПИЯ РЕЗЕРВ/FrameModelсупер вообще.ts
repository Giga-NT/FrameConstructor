...
const createRoof = useCallback((
  ...
  // Рассчитываем реальные размеры крыши с учетом свесов
  ...
  // Начальная и конечная точки с учетом свесов
  const startX = -frontOverhang + 0.2;
  const endX = length + rearOverhang * 2;
  
  // Высоты с учетом свесов
  ...
  // Позиционирование крыши
  roofMesh.position.set(
    startX,
    height + 0.05, // Высота каркаса
    Math.min(startZ, endZ) - (startZ < endZ ? leftSideOverhang : rightSideOverhang)
  );
  
  // Если Z координаты идут в обратном порядке, разворачиваем крышу
  if (startZ > endZ) {
