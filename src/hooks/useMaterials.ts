...
// Добавляем интерфейс MaterialPrices
...
interface WeldingInfo {
    totalConnections: number;
    columnWeldingCost: number;
    trussWeldingCost: number;
    purlinWeldingCost: number;
    totalWeldingCost: number;
    weldType: number;
}

interface UseMaterialInfoResult {
...
export const useMaterialInfo = (
    ...
    return useMemo(() => {
        const {
            ...
            topPurlinSpacing, numBaysLeft = 0, numBaysRight = 0 // Добавляем значения по умолчанию
        } = frameParams;
        ...
        const trussConnections = trussCount * ((numBaysLeft ?? 0) + (numBaysRight ?? 0) + 1) * 3;
        ...
        // Получаем цены на материалы (с проверкой на undefined)
        ...
        // Получаем цену сварки (с проверкой на undefined)
